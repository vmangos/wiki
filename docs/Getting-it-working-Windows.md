...
Insert a record for your realm into the [`realmlist`](realmd/realmlist.md) table of the `realmd` database. You can do this via a query in your GUI MySQL client or via the command line.
...
**Using a GUI client:**
Open the `realmd` database, find the [`realmlist`](realmd/realmlist.md) table, and add a new row with your desired values.
...
2. **Edit the database connection strings**
   Open both files in Notepad or any text editor and locate the lines that begin with `LoginDatabase.Info`, `WorldDatabase.Info`, `CharacterDatabase.Info`, and `LogsDatabase.Info`. The default entries look like:
