...
Open the "mangosd.conf" file with Notepad and change the _LogLevel.Console_ and _LogLevel.File_ settings to 0.
...
Make sure the server has finished loading. If you can't type in the mangosd console then it's still loading. If you are trying to connect from another computer, then you have to change the realm's IP address to the real IP of the host machine. This can be done in the `realmd` database, inside the [`realmlist`](realmd/realmlist.md) table.

### I can't enter the Ahn'Qiraj raid.

This happens because the gate opening event has not been completed on your server. Go to the [`variables`](world/variables.md) table in the world database and set variable 30050 to 12.

### I am trying to start a game event, but it is disabled.

You need to use _.event enable_ before _.event start_ when the event is disabled in the database. Alternatively you can just remove the disabled flag from the event in the [`game_event`](world/game_event.md) table.
